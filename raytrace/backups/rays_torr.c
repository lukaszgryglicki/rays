...
/*    printf("alfa = %f, beta = %f\n", (acos(a2)*180.)/(3.1415926), (beta*180.)/(3.1415929));*/
    ...
    pn.y = -(n->y * cos(beta));
    pn.z = -(n->z * cos(beta));
    pv.x = (a2*n->x - obsV.x)*(t->mU/t->mD);
    pv.y = (a2*n->y - obsV.y)*(t->mU/t->mD);
    pv.z = (a2*n->z - obsV.z)*(t->mU/t->mD);
    /*printf("spr = %f\n", scalar_prod(&pn, &pv));
    printf("n = (%f,%f,%f)\n", n->x, n->y, n->z);
    printf("pn = (%f,%f,%f)\n", pn.x, pn.y, pn.z);
...
/*    if (beta < 3.14 / 4.) printf("alfa = %f, beta = %f\n", (acos(a2)*180.)/(3.1415926), (beta*180.)/(3.1415929));*/
    ...
    pn.y = n->y * cos(beta);
    pn.z = n->z * cos(beta);
    pv.x = (a2*in.x - obsV.x)*(t->mD/t->mU);
    pv.y = (a2*in.y - obsV.y)*(t->mD/t->mU);
    pv.z = (a2*in.z - obsV.z)*(t->mD/t->mU);
