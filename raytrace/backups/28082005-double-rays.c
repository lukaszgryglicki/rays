...
/* print_ray(r);*/
 if (r->r >= max_rec)
   {
    if (r->c == RED)   return (double)BACK_R/511.;
    if (r->c == GREEN) return (double)BACK_G/511.;
    if (r->c == BLUE)  return (double)BACK_B/511.;
   }
 ...
 *recfr = NULL;
 ...
 while ((u = getopt(lb,par,"ghHK2BLb:t:r:R:o:S:p:P:i:x:y:D:X:Y:m:a:q:AJG"))!=-1)
 {
  switch (u)
   {
