...
void read_transformation(FILE* f, REAL*** mm, REAL*** mn, REAL*, int* tuse);
...
int read_triangle(FILE* f, Triangle* t, int idx, int* t_use, char** tmap)
{
 int nr,n,ti;
 int t_used;
 int i,ok;
 ...
 if (nr == 1 && !strcmp(str, "Transform:"))
    {
     ...
     trmatn = matrix(4);
     I_matrix(trmatn, 4);
     read_transformation(f, &trmat, &trmatn, &dd, &t_used);
     if (dd >= 0.) t->n_dist = dd;
    }
...
void read_transformation(FILE* f, REAL*** mm, REAL*** mn, REAL* nor_dist, int* tuse)
{
 int done;
 int nr;
 REAL par,par2,par3;
...
/*	   printf("Observer transform.\n");*/
	...
	I_matrix(lm, 4);
	I_matrix(lmn, 4);
 	read_transformation(f, &lm, &lmn, &dd, &i);
 ...
 if (nr == 1 && !strcmp(str, "WorldTransform:"))
       {
	read_transformation(f, &world, &worldn, &dd, &trans_used);
 ...
 *ts = (Triangle*)malloc(n*sizeof(Triangle));
 ...
 for (i=0;i<n;i++)
   {
    if (!read_triangle(f, &((*ts)[i]), i, t_use, tmap)) spanic("load_scene: general triangle read failure", HERE);
 ...
 for (i=idx;i<s->x;i++)
   {
    ...
    load1 += (REAL)proc_tr;
    load2 += (REAL)proc_tr2;
    printf("%3.1Lf%%,%3.1Lf%%,%dK/%dK\n", ((REAL)(i+1)*100.)/(REAL)s->x,(REAL)(proc_tr*100)/(REAL)proc_tr2,proc_tr/1000,proc_tr2/1000);
    if (i > 0 && !(i % bkup))
      {
...
/*    printf("\n");*/
 ...
 printf("\n");
 printf("Final Load: %3.3Lf%%,%13.0Lf/%13.0Lf\n", (load1*100.)/load2,load1,load2);
...
void help(char* fn)
{
...
#ifndef NOGL
 printf("\t-t N; time period to OGL flush      [2000 in milisec]\n");
#endif
 ...
 printf("\t-T texture prefix search            [off,on=\"/usr/share/\"]\n");
 ...
 printf("graoup options: -L(-S-D), -N(-n-s), -G(-t), -J(-q-g), -A(-K-2)\n");
#ifndef NOGL
 ...
 printf("\tb/B:    maiputale backup per lines\n");
 ...
 *recfr = NULL;
 ...
 strcpy(tprefix, "");
 while ((u = getopt(lb,par,"ghHNK2BLAJGb:t:r:R:o:S:p:P:i:x:y:D:X:Y:m:a:q:s:n:T:"))!=-1)
 {
  switch (u)
   {
    ...
    case 'R':  *recfr = malloc(strlen(optarg)+2); strcpy(*recfr, optarg); break;
...
{
 glViewport(0, 0, (GLsizei)thr_sx, (GLsizei)thr_sy);
