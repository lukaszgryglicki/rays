...
void get_normal(Vector* n, Triangle* t, Vertex* p, Material* R, Material* G, Material* B, TexCoord* tex)
{
 ...
 err = 0;
 la = lb = lc = 0.;
 if ((sol = line_intersect(&pa, &bc, &ap)) != 1) { la = 1.; lb = lc = 0.; err = 1; printf("+"); }
 if ((sol = line_intersect(&pb, &ca, &bp)) != 1) { lb = 1.; la = lc = 0.; err = 1; printf("+"); }
 if ((sol = line_intersect(&pc, &ab, &cp)) != 1) { lc = 1.; la = lb = 0.; err = 1; printf("+"); }
 if (!err)
   {
    la = distance(&pa, p);
    lb = distance(&pb, p);
