...
void calc_normal(double u, double v, double* x, double* y, double* z)
{
 ...
 if (1 - 2.*u <= epsu) epsu *= -1.;
 if (1 - 2.*v <= epsv) epsv *= -1.;
   ...
   for (j=0;j<epsV;j++)
    {
     ...
     fprintf(plik, "%f %f %f\n", s*pts[i][j][0], s*pts[i][j][1], s*pts[i][j][2]);*/
       ...
       fprintf(plik,"transparencyA: RGB: (%f,%f,%f)\n", tr, tg, tb);
       fprintf(plik,"specularA: RGB: (%f,%f,%f)\n", sr, sg, sb);
       fprintf(plik,"diffuseA: RGB: (%f,%f,%f)\n", cr, cg, cb);
       fprintf(plik,"transparencyB: RGB: (%f,%f,%f)\n", tr, tg, tb);
       fprintf(plik,"specularB: RGB: (%f,%f,%f)\n", sr, sg, sb);
       fprintf(plik,"diffuseB: RGB: (%f,%f,%f)\n", cr, cg, cb);
       fprintf(plik,"transparencyC: RGB: (%f,%f,%f)\n", tr, tg, tb);
       fprintf(plik,"specularC: RGB: (%f,%f,%f)\n", sr, sg, sb);
       fprintf(plik,"diffuseC: RGB: (%f,%f,%f)\n", cr, cg, cb);
       fprintf(plik,"surface: ABCD: (0,0,0,0)\n");
       fprintf(plik,"transparencyFact: (1,%f)\n", tf);
