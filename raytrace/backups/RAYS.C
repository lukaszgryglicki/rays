 ...
 if (!aa)
   {
    ...
    for (j=0;j<sx;j++)
      {
       pix[sx*((sy-i)-1)+j] =
	   (int)(0.299*(double)screen.pixels[3*(sy*j+i)])+
	   (int)(0.587*(double)screen.pixels[3*(sy*j+i)+1])+
	   (int)(0.114*(double)screen.pixels[3*(sy*j+i)+2]);
      }
   }
 else
   {
    ...
    for (j=0;j<sx;j++)
	   ...
	   (((int)(0.299*(double)screen.pixels[3*(2*sy*(2*j)+(2*i))])+
	   (int)(0.587*(double)screen.pixels[3*(2*sy*(2*j)+(2*i))+1])+
	   (int)(0.114*(double)screen.pixels[3*(2*sy*(2*j)+(2*i))+2]))+
	   ((int)(0.299*(double)screen.pixels[3*(2*sy*(2*j+1)+(2*i))])+
	   (int)(0.587*(double)screen.pixels[3*(2*sy*(2*j+1)+(2*i))+1])+
	   (int)(0.114*(double)screen.pixels[3*(2*sy*(2*j+1)+(2*i))+2]))+
	   ((int)(0.299*(double)screen.pixels[3*(2*sy*(2*j)+(2*i+1))])+
	   (int)(0.587*(double)screen.pixels[3*(2*sy*(2*j)+(2*i+1))+1])+
	   (int)(0.114*(double)screen.pixels[3*(2*sy*(2*j)+(2*i+1))+2]))+
	   ((int)(0.299*(double)screen.pixels[3*(2*sy*(2*j+1)+(2*i+1))])+
	   (int)(0.587*(double)screen.pixels[3*(2*sy*(2*j+1)+(2*i+1))+1])+
	   (int)(0.114*(double)screen.pixels[3*(2*sy*(2*j)+(2*i))+2])))/4;
   }
}
...
double** matrix(int siz)
{
 double** mem;
 int i;
 if (siz <= 0) return NULL;
 mem = (double**)malloc(siz*sizeof(double*));
 for (i=0;i<siz;i++) mem[i] = vector(siz);
 return mem;
}


void I_matrix(double** dst, int siz)
{
 int i,j;
...
double** matrix_mul(double** m, double** n, int ma, int mb, int na, int nb)
{
 double** dst;
 int k,j,i;
 if (ma <= 0 || mb  <= 0 || na <= 0 || nb <=0 || mb != na) return NULL;
 if (!m || !n) return NULL;
 dst = (double**)malloc(ma*sizeof(double*));
 if (!dst) return NULL;
 for (i=0;i<ma;i++) dst[i] = (double*)malloc(nb*sizeof(double));
 for (i=0;i<ma;i++)
 for (j=0;j<nb;j++)
    {
     dst[i][j] = 0.0;
...
void matrix_mul_vector(double* dst, double** m, double* v, int len)
{
 int i,j;
...
void free_matrix(double** m, int siz)
{
 int i;
 if (siz <= 0) return;
...
void copy_matrix(double** dst, double** src, int siz)
{
 int i,j;
...
int try_swap(double** m, int idx, int dim)
{
 int x;
...
double** invert_matrix(double** srcC, int dim)
{
 double** src, **dst;
 double div, pom;
 register int x,k;
...
void rotatex(double** m, double ang)
{
 ang /= 180./PI;
 m[1][1] = cos(ang);
 m[2][1] = sin(ang);
...
void rotatey(double** m, double ang)
{
 ang /= 180./PI;
 m[0][0] = cos(ang);
 m[2][0] = -sin(ang);
...
void rotatez(double** m, double ang)
{
 ang /= 180./PI;
 m[0][0] = cos(ang);
 m[1][0] = sin(ang);
...
void translatex(double** m, double arg)
{
 m[0][3] = arg;
}


void translatey(double** m, double arg)
{
 m[1][3] = arg;
}


void translatez(double** m, double arg)
{
 m[2][3] = arg;
}
...
void scalex(double** m, double arg)
{
 m[0][0] = arg;
}


void scaley(double** m, double arg)
{
 m[1][1] = arg;
}


void scalez(double** m, double arg)
{
 m[2][2] = arg;
}


void scale(double** m, double x, double y, double z)
{
 scalex(m, x);
 scaley(m, y);
 scalez(m, z);
...
void print_matrix(double** m, int siz)
{
 int i,j;
 ...
 for (i=0;i<siz;i++)
   {
     for (j=0;j<siz;j++) printf("%3.4f\t", m[i][j]);
    printf("\n");
   }
}
...
void transform_observer(Vertex* t, double** m, double** nm)
{
 double *x, *rx;
...
void transform_light(void* t, double** m, double** nm, int is_vec)
{
 double *x, *rx;
 double **M;
 Vector* v;
...
void transform_triangle(Triangle* t, double** m, double** nm)
{
 double *x, *rx;
...
void read_transformation(FILE* f, double*** mm, double*** mn, int* tuse);
 ...
 if (nr == 1 && !strcmp(str, "Transform:"))
    {
     ...
     trmatn = matrix(4);
     I_matrix(trmatn, 4);
     read_transformation(f, &trmat, &trmatn, &t_used);
    }
...
/* int nTex;*/
 ...
 fread(&lookz,sizeof(double),1,f);
 fread(&i,sizeof(int),1,f);
 if (!ovr_b) bkup = i;
 fread(&i,sizeof(int),1,f);
 if (!ovr_r) max_rec = i;
 fread(&dd,sizeof(double),1,f);
...
/* int nTex;*/
 ...
 fwrite(&lookz,sizeof(double),1,f);
 fwrite(&bkup,sizeof(int),1,f);
 fwrite(&max_rec,sizeof(int),1,f);
 fwrite(&minshadow,sizeof(double),1,f);
 fwrite(&ambient,sizeof(double),1,f);
...
void m_ownmatrix(double*** m, double** mat)
{
 double** newm;
...
void m_translate(double*** m, double x, double y, double z)
{
 double** trans;
 double** newm;
 trans = matrix(4);
...
void m_scale(double*** m, double x, double y, double z)
{
 double** scal;
 double** newm;
 scal = matrix(4);
...
void m_rotatex(double*** m, double ang)
{
 double** rot;
 double** newm;
 rot = matrix(4);
...
void m_rotatey(double*** m, double ang)
{
 double** rot;
 double** newm;
 rot = matrix(4);
...
void m_rotatez(double***m, double ang)
{
 double** rot;
 double** newm;
 rot = matrix(4);
...
/*	   printf("Observer transform.\n");*/
	...
	I_matrix(lm, 4);
	I_matrix(lmn, 4);
 	read_transformation(f, &lm, &lmn, &i);
 ...
 if (nr == 1 && !strcmp(str, "WorldTransform:"))
	read_transformation(f, &world, &worldn, &trans_used);
 else fseek(f, pos, SEEK_SET);
...
void help(char* fn)
{
 ...
 printf("\tr/t:    toggle terminate RT\n");
 printf("\tr/h:    display help\n");
 printf("\tr/n:    toggle function interpolating normal, DANGEROUS\n");
 printf("\tr/l:    toggle function computing intersection, DANGEROUS\n");
#endif
}
 ...
 *recfr = NULL;
 ...
 while ((u = getopt(lb,par,"ghH2BLb:t:r:R:o:S:p:P:i:x:y:D:X:Y:m:a:q:AJG"))!=-1)
 {
  switch (u)
   {
